...
class CreateNewAccount: UIViewController, UITableViewDelegate, UITextFieldDelegate {
    ...
    func handleSignUp(successAction: @escaping() -> Void) {
        guard let email = emailTextField.text else { return }
        guard let pass = newPasswordTextField.text else { return }
        
        // add the quest1 textf to your quest tf array
        // add the answer1 textf to your ansers tf array
        questionTextFields.insert(questionOneTextField, at: 0)
        answerTextFields.insert(answerOneTextField, at: 0)
        
        // -> create a dictionary from quest array and answer arra
        var questionDict = [String: String]()
        for i in 0 ..< questionTextFields.count {
            guard let q = questionTextFields[i].text?.trimmingCharacters(in: .whitespacesAndNewlines),
                let a = answerTextFields[i].text else { continue }
            questionDict[q] = a.trimmingCharacters(in: .whitespacesAndNewlines)
        }
        
        // -> check if count of dict >= 3 -> less than -> show error not enough quest
        if questionDict.count < 3 {
            securityQuestionsErrorLabel.text = "Please check your questions, duplicated or sth "
            return
        }
